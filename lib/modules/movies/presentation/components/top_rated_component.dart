...
class TopRatedComponent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // LoggerDebug.loggerInformationMessage("BuildContext TopRatedComponent");

    return BlocBuilder<MoviesBloc, MoviesState>(
        buildWhen: (previous, current) => previous.topRatedState != current.topRatedState,
        builder: (context, state) {
          // LoggerDebug.loggerInformationMessage("BlocBuilder TopRatedComponent + ${state.topRatedState}");
          switch (state.topRatedState) {
            ...
            case RequestState.error:
              return SizedBox(height: 170.0, child: Center(child: Text(state.topRatedMessage)));
          }
        });
  }
}
